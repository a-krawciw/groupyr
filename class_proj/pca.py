...
def logistic_pca(X, num_components=None, num_iter=32):
    ...
    num_components = min(num_samples, num_dimensions) if num_components is None else num_components
    # shorthands
    N = num_samples
    D = num_dimensions
    K = num_components
    ...
    for iter in range(num_iter):
        ...
        # 2.optimise the variational parameters in order to make the
        ...
        for n in prange(N):
            ...
            E_zz = C[:, :, n] + z@z.T
            # xi squared
            xixi = np.sum(W*(W@E_zz), axis=1) + 2*b*(W@z) + b**2
            # update
...
if __name__ == '__main__':
    print(logistic_pca(np.array([(0, 1, 2), (2, 3, 4)])))
    njit(logistic_pca(np.array([(0, 1, 2), (2, 3, 4)])))
